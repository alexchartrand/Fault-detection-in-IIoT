...
currantTrain = [c.simData().getCol("currant") for c in train]
currantVal = [c.simData().getCol("currant") for c in validation]

# Filter train data
cutoff_hz = 200
order = 3
freq = 1.0 / motors[0].SAMPLING_TIME

#currantTrain = [filt.applyButterLowpassFilter(c, cutoff_hz, freq, order) for c in currantTrain]

# Remove transitional state
currantTrain = [c[steady_state_start:] for c in currantTrain]
